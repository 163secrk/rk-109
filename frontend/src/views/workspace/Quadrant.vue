<template>
  <div class="quadrant-page">
    <div class="page-header">
      <div class="page-title">任务四象限</div>
    ...
    <div class="quadrant-wrapper">
      ...
      <div class="quadrant-grid">
        <div
          class="quadrant-cell q1"
          :class="{ 'drag-over': dragOverQuadrant === 'q1' }"
          @dragover.prevent="handleDragOver('q1')"
          @dragleave="handleDragLeave"
          @drop="handleDrop('q1')"
        >
          <div class="quadrant-header">
            <span class="quadrant-title">第一象限</span>
            <span class="quadrant-subtitle">紧急 · 重要</span>
            <n-tag size="small" round type="error" :bordered="false">{{ getTasksByQuadrant('q1').length }}</n-tag>
          </div>
          <div class="quadrant-body">
            <div
              v-for="task in getTasksByQuadrant('q1')"
              :key="task.id"
              class="task-card"
              draggable="true"
            ...
            <n-empty v-if="getTasksByQuadrant('q1').length === 0" description="暂无任务" style="padding: 20px 0" />
          </div>
        </div>

        <div
          class="quadrant-cell q2"
          :class="{ 'drag-over': dragOverQuadrant === 'q2' }"
          @dragover.prevent="handleDragOver('q2')"
          @dragleave="handleDragLeave"
          @drop="handleDrop('q2')"
        >
          <div class="quadrant-header">
            <span class="quadrant-title">第二象限</span>
            <span class="quadrant-subtitle">不紧急 · 重要</span>
            <n-tag size="small" round type="primary" :bordered="false">{{ getTasksByQuadrant('q2').length }}</n-tag>
          </div>
          <div class="quadrant-body">
            <div
              v-for="task in getTasksByQuadrant('q2')"
              :key="task.id"
              class="task-card"
              draggable="true"
              @dragstart="handleDragStart(task)"
            ...
            <n-empty v-if="getTasksByQuadrant('q2').length === 0" description="暂无任务" style="padding: 20px 0" />
          </div>
        </div>

        <div
          class="quadrant-cell q3"
          :class="{ 'drag-over': dragOverQuadrant === 'q3' }"
          @dragover.prevent="handleDragOver('q3')"
          @dragleave="handleDragLeave"
          @drop="handleDrop('q3')"
        >
          <div class="quadrant-header">
            <span class="quadrant-title">第三象限</span>
            <span class="quadrant-subtitle">紧急 · 不重要</span>
            <n-tag size="small" round type="warning" :bordered="false">{{ getTasksByQuadrant('q3').length }}</n-tag>
          </div>
          <div class="quadrant-body">
            <div
              v-for="task in getTasksByQuadrant('q3')"
              :key="task.id"
              class="task-card"
              draggable="true"
              @dragstart="handleDragStart(task)"
            ...
            <n-empty v-if="getTasksByQuadrant('q3').length === 0" description="暂无任务" style="padding: 20px 0" />
          </div>
        </div>

        <div
          class="quadrant-cell q4"
          :class="{ 'drag-over': dragOverQuadrant === 'q4' }"
          @dragover.prevent="handleDragOver('q4')"
          @dragleave="handleDragLeave"
          @drop="handleDrop('q4')"
        >
          <div class="quadrant-header">
            <span class="quadrant-title">第四象限</span>
            <span class="quadrant-subtitle">不紧急 · 不重要</span>
            <n-tag size="small" round type="default" :bordered="false">{{ getTasksByQuadrant('q4').length }}</n-tag>
          </div>
          <div class="quadrant-body">
            <div
              v-for="task in getTasksByQuadrant('q4')"
              :key="task.id"
              class="task-card"
              draggable="true"
              @dragstart="handleDragStart(task)"
            ...
            <n-empty v-if="getTasksByQuadrant('q4').length === 0" description="暂无任务" style="padding: 20px 0" />
          </div>
        </div>
      </div>
    </div>

    <n-modal v-model:show="showCreateModal" :mask-closable="false" preset="dialog" title="新建个人任务" :show-icon="false">
      <n-form ref="createFormRef" :model="createForm" label-placement="top">
        ...
        <n-form-item label="任务描述">
          <n-input v-model:value="createForm.description" type="textarea" :rows="3" placeholder="请输入任务描述" />
        </n-form-item>
        <n-form-item label="选择象限" path="quadrant" :rule="{ required: true, message: '请选择象限' }">
          <n-radio-group v-model:value="createForm.quadrant">
            <n-space :size="12" wrap>
              <n-radio value="q1">
                <n-tag size="small" round type="error" :bordered="false">紧急 · 重要</n-tag>
              </n-radio>
              <n-radio value="q2">
                <n-tag size="small" round type="primary" :bordered="false">不紧急 · 重要</n-tag>
              </n-radio>
              <n-radio value="q3">
                <n-tag size="small" round type="warning" :bordered="false">紧急 · 不重要</n-tag>
              </n-radio>
              <n-radio value="q4">
                <n-tag size="small" round type="default" :bordered="false">不紧急 · 不重要</n-tag>
              </n-radio>
            </n-space>
          </n-radio-group>
        </n-form-item>
      ...
      <n-drawer-content v-if="currentTaskDetail" :title="currentTaskDetail.task.title" :closable="true">
        <div class="task-detail">
          <n-form :model="editTaskForm" label-placement="top">
            ...
            <n-space :size="16" style="width: 100%">
              <n-form-item label="紧急度" style="flex: 1">
                <n-select
                  v-model:value="editTaskForm.urgency"
                  :options="levelOptions"
                  style="width: 100%"
                />
              </n-form-item>
              <n-form-item label="重要度" style="flex: 1">
                <n-select
                  v-model:value="editTaskForm.importance"
                  :options="levelOptions"
                  style="width: 100%"
                />
              </n-form-item>
...
const quadrantMap = {
  q1: { urgency: 'high', importance: 'high' },
  q2: { urgency: 'low', importance: 'high' },
  q3: { urgency: 'high', importance: 'low' },
  q4: { urgency: 'low', importance: 'low' },
}

const getTasksByQuadrant = (quadrant) => {
  const { urgency, importance } = quadrantMap[quadrant]
  return tasks.value.filter(
    (t) => t.urgency === urgency && t.importance === importance
  )
}
...
const fetchTasks = async () => {
  loading.value = true
  try {
    const res = await listMyTasksApi()
    tasks.value = res
  } catch (e) {
...
const handleCreateTask = async () => {
  ...
  try {
    const { urgency, importance } = quadrantMap[createForm.quadrant]
    const payload = {
    ...
    await createPersonalTaskApi(payload)
...
const openTaskDetail = async (task) => {
  try {
    ...
    editTaskForm.urgency = res.task.urgency
    editTaskForm.importance = res.task.importance
...
const handleDrop = async (quadrant) => {
  ...
  const { urgency, importance } = quadrantMap[quadrant]
  if (draggingTask.value.urgency === urgency && draggingTask.value.importance === importance) {
    dragOverQuadrant.value = null
    return
  }
  try {
    await updateTaskApi(draggingTask.value.id, { urgency, importance })
    message.success('已更新象限')
    await fetchTasks()
  } catch (e) {
    handleError(e, '更新失败')
  } finally {
